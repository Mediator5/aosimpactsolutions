      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 400,
            ...
            Your Path to a{' '}<em style={{ color: 'var(--color-gold-light)' }}>Stronger Credit Profile</em>
          </h2>
        </div>
              ...
              return (
                <div key={i} data-step style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '2rem',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'var(--color-black)',
                      border: '2px solid var(--color-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      boxShadow: '0 0 20px rgba(201,168,76,0.2)', position: 'relative', zIndex: 1 }}>
                      <span style={{ fontFamily: 'var(--font-heading)', fontSize: '1.1rem', fontWeight: 600, color: 'var(--color-gold)' }}>{step.number}</span>
                    </div>
                  </div>
...
function StepCard({ step, align }) {
  return (
    <div style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)',
      ...
      transition: 'border-color 0.3s ease', maxWidth: '420px',
