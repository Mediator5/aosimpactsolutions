...
/* ── NEGATIVE ITEMS ── */
const negativeItems = ['Late Payments','Collections','Charge-Offs','Medical Collections','Repossessions','Hard Inquiries','Bankruptcy Reporting Errors','Identity Theft Accounts','Incorrect Personal Information']
...
export function NegativeItemsSection() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver((entries) => {
      entries.forEach((e) => {
        if (e.isIntersecting) e.target.querySelectorAll('[data-item]').forEach((el, i) => {
  ...
  return (
    <section ref={ref} style={{ background: 'var(--color-surface)', padding: '7rem 1.5rem', position: 'relative', overflow: 'hidden' }}>
      {/* Unsplash: documents / paperwork */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px,1fr))', gap: '5rem', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.9rem', fontWeight: 300,
              ...
              We help challenge inaccurate, outdated, or questionable items on your credit report.
              Every profile is unique — your strategy will be customized based on your specific situation.
            </p>
            <Link href="/consultation" className="btn-primary">See What We Can Remove</Link>
          </div>

          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
            {negativeItems.map((item, i) => (
              <div key={i} data-item style={{ padding: '0.6rem 1.2rem', background: 'var(--color-surface-2)',
      ...
      <style>{`.item-revealed { opacity:1!important; transform:scale(1)!important; }`}</style>
    </section>
  )
}
...
export function WhoWeHelpSection() {
  return (
    <section style={{ background: 'var(--color-black)', padding: '7rem 1.5rem', position: 'relative', overflow: 'hidden' }}>
      {/* Unsplash: aspirational family / community */}
      <div style={{ position: 'absolute', inset: 0,
      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: 'var(--color-surface-2)', border: '1px solid rgba(220,159,37,0.3)',
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 300, color: 'var(--color-muted)' }}>
...
/* ── FAQ ── */
const faqs = [
  { q: 'How long does credit repair take?', a: 'Most clients begin seeing results within 30–90 days. Full results typically take 2–6 months depending on your credit profile and the number of negative items.' },
  { q: 'Can you guarantee items will be removed?', a: 'No company can legally guarantee specific results. We use a strategic and compliant dispute process to achieve the best possible outcome for every client.' },
  { q: 'What negative items can you help remove?', a: 'We help challenge collections, charge-offs, late payments, medical bills, inquiries, repossessions, bankruptcy reporting errors, and incorrect personal information.' },
  { q: 'Do I need credit monitoring?', a: 'Yes. Credit monitoring allows us to access your reports, track progress, and respond quickly throughout the process. It is included with all programs.' },
  { q: 'Will this improve my credit score?', a: 'As negative items are corrected or removed, many clients see improvements. Results vary based on your overall credit profile and existing positive accounts.' },
  { q: 'Do I need to do anything during the process?', a: 'Yes. You may need to provide documents and follow guidance such as avoiding new credit inquiries to maximize results.' },
  { q: 'What happens if I miss a payment?', a: 'For month-to-month plans, services are paused if a payment is missed and resume once the account is brought current. Contracts are terminated if payment is missed within the first three months.' },
  { q: 'Is credit repair legal?', a: 'Yes. You have the legal right to dispute inaccurate or incomplete information on your credit report, and we assist you in exercising those rights.' },
]
            ...
            return (
              <div key={i} style={{ background: 'var(--color-surface-2)',
                ...
                <div style={{ maxHeight: isOpen ? '300px' : '0', overflow: 'hidden', transition: 'max-height 0.35s ease' }}>
                  <div style={{ padding: '0 1.5rem 1.25rem', fontFamily: 'var(--font-body)', fontSize: '0.86rem',
                    fontWeight: 300, color: 'var(--color-muted-light)', lineHeight: 1.85,
                    borderTop: '1px solid var(--color-border)', paddingTop: '1rem' }}>{faq.a}</div>
                </div>
              </div>
            )
          })}
